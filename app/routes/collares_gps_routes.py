...
nuevo_collar_gps = CollarGPScontroller()


@router.post("/create_collar_gps")
async def create_collar_gps(collares_gps: Collares_GPS):
    rpta = nuevo_collar_gps.create_collar_gps(collares_gps)
    return rpta
...
async def get_collargps(collargps_id: int):
    try:
        rpta = nuevo_collar_gps.get_collargps(collargps_id)
        return rpta
...
async def get_collaresgps():
    rpta = nuevo_collar_gps.get_collaresgps()
    return rpta
...
async def update_collargps(collargps_id: int, collares_gps: Collares_GPS):
    try:
        rpta = nuevo_collar_gps.update_collargps(
...
async def delete_collargps(collargps_id: int):
    try:
        rpta = nuevo_collar_gps.delete_collargps(collargps_id)
        return rpta
    except HTTPException as e:
        raise e
    except Exception as e:
