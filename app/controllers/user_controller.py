...
from models.mascotas_model import Mascotas 
...
class Usercontroller():
    def create_mascota(self, mascota: Mascotas):   
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("INSERT INTO mascota (nombre,tipo_mascota,id_propietario,coordenadas) VALUES (%s, %s, %s, %s)", (mascota.nombre, mascota.tipo_mascota, mascota.id_propietario, mascota.coordenadas))
            conn.commit()
            conn.close()
    ...
    def get_mascota(self, mascota_id: int):
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM mascota WHERE id = %s", (mascota_id,))
    ...
    def update_mascota(self,mascota_id: int, mascota: Mascotas):
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("UPDATE mascota SET nombre = %s, tipo_mascota = %s, coordenadas = %s WHERE id = %s",(mascota.nombre,mascota.tipo_mascota, mascota.coordenadas,mascota_id))
            conn.commit()
        ...
        try:
            ...
            cursor.execute(
                "UPDATE usuarios SET email = %s, password = %s, nombre = %s, apellido = %s, documento = %s, telefono = %s, id_rol = %s WHERE id = %s",
                    (user.email, user.password, user.nombre, user.apellido, user.documento, user.telefono, user.id_rol, user_id)
                    )
            conn.commit()
